...
public class MultipleVirtualViewportBuilder {
	...
	public MultipleVirtualViewportBuilder(float minWidth, float minHeight,
	...
	public VirtualViewport getVirtualViewport(float width, float height) {
		if (width >= minWidth && width <= maxWidth && height >= minHeight
				&& height <= maxHeight)
		...
		if (insideBounds(virtualViewportWidth, virtualViewportHeight))
			return new VirtualViewport(virtualViewportWidth,
					virtualViewportHeight, false);
